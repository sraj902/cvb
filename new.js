...
function getDosageDetails() {
    ...
    hideElement("removeCenterForm");
    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'new.php?getDosageDetails=true', true);
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            for (var i = 0; i < dosageData.length; i++) {
                ...
                var row = "<tr><td>" + dosage.cname + "</td><td>" + dosage.whs + "</td><td>" + dosage.whe + "</td></tr>";
...
function showRemoveCenter() {
    hideElement("addCenterForm");
    hideElement("removeCenterForm");
    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'new.php?showRemoveCenter=true', true);
    xhr.onload = function() {
        if (xhr.status === 200) {
            var dosageData = JSON.parse(xhr.responseText);
            var dosageTable = document.getElementById("dosageTable");
